...
class AgendamentoDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('motivo', function ($query) {
            return $query->observacoes ?? 'n/a';
        })
        ->addColumn('editar', function($query){

            $show = '<a class="btn btn-info" href="'.route('view.medico.show',$query->id).'" style="margin-bottom:5px"><i class=" far fa-eye"></i></a>';

            $edit = '<a class="btn btn-primary" href="'.route('view.medico.edit',$query->id).'" style="margin-bottom:5px"><i class="far fa-edit"></i></a>';

            $delete = '<a href="'.route('agendamento.destroy',$query->id).'" class="btn btn-danger delete-item"><i class="far fa-trash-alt"></i></a>';
            return $show.$edit.$delete;
        })
        ->rawColumns(['editar', 'medico','data','motivo'])
        ->setRowId('id');
    }
